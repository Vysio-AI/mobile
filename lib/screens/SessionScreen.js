...
const SessionScreen = ({navigation}) => {
  ...
  const startSession = async () => {
    ...
    const fetchSession = await fetch('http://10.0.0.12:3000/api/v1/sessions', {
      method: 'POST',
      headers: {
  ...
  const endSession = async () => {
    ...
    // End the session
    const fetchSession = await fetch(`http://10.0.0.12:3000/api/v1/sessions/${session.data.id}`, {
      method: 'PATCH',
      headers: {
        Authorization: `Bearer ${accessToken}`,
        'Content-Type': 'application/json',
    ...
    const createVideo = await fetch('http://10.0.0.12:3000/api/v1/videos', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
        'Content-Type': 'application/json',
