...
const ReferralScreen = ({navigation}) => {
  ...
  const client = useQuery(['client', userInfo.id, accessToken], () => getClient(userInfo.id, accessToken));
  ...
  useEffect(() => {
    if (client.data && client.data.practitionerId) {
      navigation.reset({
        index: 0,
        routes: [{name: 'Main'}],
      });
  ...
  if (client.isLoading || (client.data && client.data.practitionerId)) {
    return (
      <View></View>
    )
  }
